...
Matrix4 MMath::perspective(const float fovy_, const float aspect_, 
							const float zNear_, const float zFar_) {
	float cot = 1.0f / tan(fovy_ * 0.5f * DEGREES_TO_RADIANS);
	// Don't forget, this looks row centric but it really is a 
	//column matrix - right-hand rule rules
...
Matrix4 MMath::orthographic(float xMin_, float xMax_, float yMin_, 
							float yMax_, float zMin_, float zMax_) {
	Matrix4 m;

	Matrix4 m1 = MMath::scale(2.0f / (xMax_ - xMin_), 
							  2.0f / (yMax_ - yMin_), 
							 -2.0f / (zMax_ - zMin_));
	Matrix4 m2 = MMath::translate(-(xMax_ + xMin_) / (xMax_ - xMin_), 
								  -(yMax_ + yMin_) / (yMax_ - yMin_), 
								  -(zMax_ + zMin_) / (zMax_ - zMin_));
	m = m2 * m1;
...
/// The orthographic projection matrix is linear and affine but nothing else 
/// so there is has no inverse. Therefore, it is labeled singular or non-invertable.
...
/// Multiply screen coordinates by this matrix and you should get x and y world 
/// coordinates
